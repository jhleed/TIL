...
### 17.09.13 (수)
**MongoDB**
- `Replica Set`과 `Spring Boot`의 연동이 드디어 끝났습니다. MongoDB Instance에서 Replica Set을 설정해줄 때 IP를 AWS의 `Public DNS (IPv4)`에 연결해 줘야 합니다. 
- 처음에 이 작업을 시작할 때는 이 정도 데이터베이스 세팅 정도 책보고 하면 금방 하겠기 했는데, 예상외의 변수가 계속 생겨서 시간이 굉장히 지체되었습니다. (네트워크 방화벽 문제라든지, 권한 문제 등.) 인프라와 DB 쪽을 설정하는 경험이 거의 없었기에 발생한 일이라 생각합니다. 이런 것도 경험할수록 많이 늘겠지요.
