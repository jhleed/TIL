...
### History
- [2022년 3월](https://github.com/jhleed/TIL#2022년-3월)
- [2021년 4월](https://github.com/jhleed/TIL#2021년-4월)
...
- Node 진영의 테스트 도구들(NestJS, Jest, ...) 을 학습 중이다.
- 테스트 코드를 작성했으나, 멱등성을 보장하지 않는 경우는 일단 skip 처리 중이다.
- Mocking, Spy를 적절히 활용하고자 한다.
- 원래 Mocking 을 그다지 좋아하지는 않았는데, 멱등을 보장하기 위해 어쩔 수 없이 처리해야 하는 듯
- 어느 부분을 Mocking으로 처리할지, 어느 부분은 Integration 테스트를 할 지는 여전히 고민되는 부분이다.
...
- 모 커뮤니티에서 '이직을 하려는데 기술적으로 어려운 도전을 해본 경험이 없어서 고민이다' 라는 내용의 글을 봤는데 코멘트가 인상적이어서 기록해둔다.
    - '악마는 디테일에 있다. 쉬운 일이라도 디테일을 생각하고 개선방법을 더 고민한다면 어렵다. 일이 쉬웠다는건 디테일에 신경쓴 적이 없다는 건데 면접가서 꼬리질문 받다가 끝날거다'
- 와닿는 말이다. 내가 어떤 Task를 받았을때 '아 이건 시시한 Task네 .. 더 챌린징한 Task를 받고싶어 ..' 라고 생각했었던 적이 몇번 있었는데, 단순히 시시한 Task 생각하지 말고, 더 다양한 상황을 고려해보고 발전시켜볼 생각을 했다면 분명 더 성장할 수 있었을 것이다.
- 오늘부터라도 어떤 일이든 다양한 상황을 고려하면서 작업을 해보자. 특히 백엔드는 고민이 많아야 한다.

## 2021.04.03

- 오랜만에 TIL을 또 쓰네.. 최근 프로젝트를 하면서 이래저래 배우는 내용이 많다. 그 내용을 복습해야 제대로 내 것으로 만들 수 있다고 생각하기 때문에 복습 차원에서 정리한다.
- 그럼 일단 기억나는대로 최근 경험한 내용을 정리해보자. 
- MSA with Spring Cloud
    - Gateway (Zuul) - 당시에는 커스터마이징 요구사항이 있어서 직접 구축했는데, 지금은 상황을 봐서 넘겨도 될 듯
    - Spring Security OAuth2 (검색해보면 하도 오래된 자료들이 많아서 첫 구축에 삽질했다. 한 번 삽질했으니 두 번은 안 해도 될 듯
    - Ribbon (Client Side LB) - POD 수가 넉넉치 않아서, QOS를 위해 Ribbon 리서치 - 클라이언트쪽에 붙여서 부하가 적은 서버로 로드밸런싱 하는 방법. 단, 서버의 상태를 동적으로 관리해야 하기 때문에 서비스 디스커버리인 Eureka를 붙여야 한다. 따라서 관리포인트가 늘어난다는 단점이 있다. (Eureka가 설마 SPOF가 될 수도..?!)
- MQ(Kafka)를 활용한 데이터 동기화 잡 구축
    - Producer에서 고려해야 할 점
        - 이미 넘겼던 데이터를 다시 발송하지 않기 위해서 LAST SEQUENCE를 기록하는 방법
    - Spring Batch를 사용하는것보다 , Kafka Producer를 사용한 이유 (성능, 복잡도)
    - Consumer에서 고려해야 할 점
        - 예외처리 : Retry Policy - 데이터 처리하다 오류가 나면? 계속 재처리 하다가 계속 죽는 경우도 있기 때문에 적절한 RetryPolicy를 설정해주면 좋음 
        - 근데 Partition보다 Pod 수가 적어지는 상황 (ex : OpenShift 장애)이 생기면 이 Retry Count가 제대로 되지 않는 오류(?)가 있음 (Partition Offset 기준으로 카운팅 하기 때문에)
        - 그럼 일부 Retry 를 하지 못 할 수도 있는데.. 이런 경우를 처리하고 싶으면 아래 2가지 방법이 있음
           1. Consumer에서 처리할 Partiton을 명시적으로 지정하기 - 동적으로 POD 수를 관리해야 하기 때문에 Eureka와 같은 서비스 디스커버리가 필요할 것 같음 
           2. 혹은, [Dead Letter](https://gunju-ko.github.io/kafka/2020/06/17/Dead-Letter-Queue.html) 방식 사용하기
        - 그러나 위 두 방법 모두 복잡도가 높아진다는 단점이 있음
        - 다시 본론으로 돌아와서.. 복잡도가 높아지는 것과 Retry를 제대로 하지 못하는 것 중 어떤 것이 더 중요한가..를 고려해 봤을때 결국 Retry를 일부 놓치더라도 단순한 방향으로 가기로 결정
        - 이유는? 로깅, 모니터링도 타이트하게 하고 있고 무엇보다 위 상황 자체가 OpenShift 장애가 났을 경우를 가정하기 때문에 아주 적은 확률로 발생할 것이며 치명적이지 않음
...
- 첫 광고 집행은 결과적으로 실패했다. 2만 5천원을 썼고, 한푼도 벌지 못했다.
- 그렇지만 실망스럽지는 않다. 이로 인해서 배운 것들이 있기 때문이다.
- 이와는 별개로, TIL을 기록하는 것을 중단할까 고민된다. 최근에는 개발 관련된 내용이 없기 때문이다. 

## 2019.12.23 (월)

- 도서관에 가서 마케팅 관련된 서적들을 읽었다. 그러나 딱히 와닿는 내용은 없었다.
- 특히 소셜 마케팅 관련 서적은 트렌드가 너무 빠르게 변하기 때문에, 몇 년만 지나도 그 가치가 희석되는 것 같다.

## 2019.12.22 (일)

- 글쓰기와 영어가 현재 내게 가장 필요한 스킬들이라고 생각된다. 
- 글쓰기를 많이 늘리려면 많이 쓰고, 좋은 글들을 많이 봐야 한다. 
- 어떤 내용이든 빼먹지 말고 꾸준히 쓰자. 지금 이렇게 TIL 을 적는 것 처럼 말이다.
...
### 꾸준함의 능력
- ['꾸준히만 해도 무조건 상위 10%다.'](http://moneyman.kr/archives/4140?fbclid=IwAR2DcAkRCWyranZGiJt_4HXPRyd0JbMSIAUKzPT-_xjfVeAvgI4LK0RuIrk)라는 글을 읽었다. 
- 믿고 보는 머니맨 페이지의 글이지만 유달리 와닿았던 글이다. 
    - 어떤 일을 할 때, 재능이 없더라도 성공할 수 있는 방법이 있다. 바로 꾸준히 하는 것이다. 
    - 1년 내내 꾸준히 뭔가를 할 수 있다면, 상위 5% 안에 들 수 있다.
- 내가 느낀 점은 이렇다.
    - 나는 완벽주의가 있다. 단점이라고 생각하지만 잘 고쳐지지 않았다. 무조건 성공하는 방법을 알 수 있다면, 이런 두려움도 고쳐질 수 있을 것 같았다. 이 글을 읽고 실마리를 잡은 것 같다.
    - **내가 이루고자 하는 일에 대해서 매일 하루에 1번씩 작은 성취라도 남기는 것이다.** 목표는 추상적이면 안 된다. 1일 1포스팅 같은 명확하게 수치화할 수 있는 목표를 정하고 행할 것이다.
    - 얼마나 잘 했냐는 중요하지 않다. **매일 꾸준히 하는게 핵심이다.**


### SpringBoot + MongoDB JPA
- SpringBoot + MongoDB JPA 를 오랜만에 연동했다.
- 예전에도 설정 삽질을 많이 했는데, 이번에도 설정 삽질을 했다. 
    - 그래도 전보다는 덜 걸렸다.
- MongoDB는 버전이 빠르게 업데이트되어 그런지 올바른 정보를 검색하는게 어렵다는 것을 느꼈다.

## 2019.12.18 (수)

### 신규 프로젝트에 모니터링 시스템 구축
- 운영 QA를 위해 신규 프로젝트에 모니터링 시스템을 구축하고 있다. 
- 잘 구축된 모니터링 시스템은 장애 및 이슈 대응에 큰 도움이 된다는 것을 광고 서버를 운영해본 경험으로 배웠다.

### 운영 QA 
- 작업한 내용을 운영계에 반영했지만 별다른 이슈가 발생되지 않았다.
- 이렇게 스무스하게 넘어간 적은 처음인 것 같다.
- 한편으로는 긴장도 많이 되었다. 상황상 개발계를 거치지 못하고 바로 운영계에 배포해야 하는 상황이었기 때문이다.

### 아이디어
- 개발자 컨퍼런스 관련 정보를 모아주는 블로그를 만들면 어떨까 싶다. 
- 처음엔 깃허브로 했는데, 광고를 붙이고 싶어서 워드프레스로 만들려고 한다. (워드프레스도 익숙해 질 겸..)

## 2019.12.17 (화)

### 배포
- 운영에 직접 배포가 나가는 것이라 꼼꼼하게 테스트 해야 한다.
- 저녁에 배포하고 잠시 지켜봤는데, 큰 이상은 없는 듯 하다.
- 리마케팅 관련된 배포 프로세스를 메모했다. 
- `C#` 운영에서 돌아가는 버전과 로컬에서 개발한 버전이 차이가 나서 빌드 실패가 났다. 
    - 프로젝트가 `.NET 4.5`로 세팅되어 있었지만 빌드는 4.0 버전으로 되고 있었다. 
- `C# 6.0` 문법에 익숙해져 있어서.. 배포 서버의 `.NET` 버전을 올리는 방법을 고려해봐야겠다.

## 2019.12.16 (월)

- 지난 주는 개발하느라 정신이 없어서 학습 내용을 기록은 못 했다. 
- 그래도 앞으로는 퇴근 전에 잠깐이라도 짬을 내어 기록하도록 하자.

### 사내 클라우드 서버에 어플리케이션 배포

- 기존에 다양한 개발 플랫폼들이 내장되어 있어서 버튼 하나로 배포 및 확장이 가능하다. 
- 개발자가 점점 개발에 집중할 수 있는 환경으로 변하고 있다. 
- 한편으로는 인프라의 진입장벽이 많이 낮아졌기 때문에 어느정도 지식은 백엔드 개발자의 필수 소양이 되고 있는 것 같기도 하다.

### QA 준비

- QA 전날이 되었는데 몇 가지 요구사항을 놓친 부분과 버그가 있어서 수정했다.
- 변명하고자 하는 것은 아니고, 어느 정도 예상된 수순이긴 했다. 현실은 항상 아름답지만은 않으니까. 
- 다만, 이러한 상황을 미리 예측하고 있었기에 좀 더 유연한 마인드로 작업이 가능했고,이건 좋은 것이라고 생각한다.

## 2019.12.09 (월)

### 사내 클라우드 서버에 어플리케이션 배포

- `OpenShift` 기반 사내 클라우드 서비스(Paas)에 처음으로 어플리케이션을 배포했다. 
    - `OpenShift`
        - 레드햇에서 만든 **컨테이너 기반 소프트웨어의 디플로이 및 관리**를 위한 제품이다.
        - 도커 컨테이너와 데브옵스 도구를 사용하는 쿠버네티스의 지원 배포판이라 할 수 있다.
...
- TDD 의 장점과도 맥이 닿아있다. 
- 하루 정도 시도해본 결과 매우 괜찮은 듯 하다.
    - 야크 쉐이빙을 완전히 없애지는 못했지만, 그래도 많이 줄어들었다. 
    - 주석도 전보다 더 명확하게 달 수 있다. 지금 내가 뭘 위해 코드를 만지고 있는지 명확하기 때문이다. 
