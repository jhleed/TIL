...
- 또 몇년만에 TIL을 쓰는지.. 최근 프로젝트를 하면서 이래저래 배우는 내용이 많다. 그 내용을 복습해야 제대로 내 것으로 만들 수 있다고 생각하기 때문에 복습 차원에서 정리한다.
- 그럼 최근 학습한 내용을 정리해보자.
- MSA with Spring Cloud
    - Zuul (Gateway) - 당시에는 커스터마이징 요구사항이 있어서 직접 구축했는데, 지금은 상황을 봐서 넘겨도 될 듯
