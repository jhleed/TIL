...
- `mocha` 테스팅 라이브러리를 이용해서 `TDD`를 하는 방법을 알아보았습니다.
- `IntelliJ` 에서는 `mocha`를 지원해주기때문에 편하게 테스트가 가능합니다. 단, 상용 버전이고 `NodeJS Plugin`이 설치 되어 있어야 합니다.
- `mocha`는 `BDD` 와 `TDD` 를 모두 지원하는데 개념적인 차이 이외에 어떤 기능적인 차이가 있는지 아직 모르겠어서 알아봐야 할 것 같습니다.
...
- `Annotation`, `Reflection`과 같이 사용법은 알지만 근본적인 존재 이유를 모르고 사용하는 기술이 

**input tag file copy**
- 하나의 `<form>` 에서 다른 `<form>`으로 파일을 복사하려면 아래처럼 간단한 코드로 처리가 가능합니다. 팀원분을 도와줄 수 있어서 좋았습니다.
```
...
**IntelliJ**
- [Module 끼리 Grouping 을 하는 방법](https://www.jetbrains.com/help/idea/grouping-modules.html)을 찾아내었습니다. 좀 더 편해진 느낌이 듭니다.
...
- token을 클라이언트 단으로 저장하는 것은 좋지 않은 것을 알고 있지만 아래와 같은 이유로 브라우저단에 보관하려고 합니다. 현재는 이렇고, 이후에 생각이 바뀔 수도 있을 것 같습니다.
	...
	3. 우리 도메인의 특성상 하나의 컴퓨터 (브라우저)를 여러 유저가 사용하지 않는다.
...
- 개인적으로 서버를 구축하는데에 동적 언어를 사용하는 것을 싫어하는 편입니다. 안정성이 중요한 서버에서는 아직까지는 역시 자바를 쓰는 것이 낫지 않나 싶습니다. Spring Boot를 사용하면 생산성도 그렇게 떨어지지 않는 것 같구요.
