...
- 이베이에 온 뒤로는 `.NET` 환경에서만 개발해서 복습도 할 겸 오랜만에 `Spring Boot` 와 `JPA를 써서 간단한 API 서버를 만들어 보았습니다. 오랜만에 사용해보니 생각보다 삽질을 많이 했습니다. 
  - `Lombok` 플러그인은 IntelliJ에도 별도로 설치를 해줘야 컴파일 단계에서 인식할 수 있음
 - `Json` 형태의 `ResponseBody`를 원했는데 Getter를 사용하지 않아서 값이 계속 안 들어옴
- `Testing`에 대한 고민 - DB와의 의존성을 느슨하기 위한 방법들
  - Repository단에서는 실제 Connection을 사용, 비즈니스 로직 단계에서는 `Mocking` 을 사용
...
- 이런 작은 규모에도 `MSA`를 적용하는 이유는 아래와 같습니다.
  - 경험
  - <u>추후 다른 개인 프로젝트에서 해당 모듈들을 재활용하기 위함</u>
